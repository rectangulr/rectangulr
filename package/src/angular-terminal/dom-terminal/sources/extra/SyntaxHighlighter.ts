...
export class SyntaxHighlighter extends TextLayout {
    constructor() {
        ...
        this.grammar = {
            ...
            load: (name, data) => {
                if (isString(data) || data instanceof Buffer) data = JSON.parse(data.toString())

                if (!isPlainObject(data))
                    throw new Error(
                        `Failed to execute 'grammar.load': Parameter 1 is not of type 'object'.`
                    )
            ...
            use: name => {
                if (!isNull(name) && isUndefined(this.grammar.registry.get(name)))
                    throw new Error(
                        `Failed to execute 'grammar.use': '${name}' is not a valid grammar name.`
                    )

                if (!isNull(name)) this.grammar.active = this.grammar.registry.get(name)
        ...
        this.theme = {
            ...
            load: (name, data) => {
                if (isString(data) || data instanceof Buffer) data = plist.parse(data.toString())

                if (!isPlainObject(data))
                    throw new Error(
                        `Failed to execute 'theme.load': Parameter 1 is not of type 'object'.`
                    )
            ...
            use: name => {
                if (!isNull(name) && isUndefined(this.theme.registry.get(name)))
                    throw new Error(
                        `Failed to execute 'theme.use': '${name}' is not a valid theme name.`
                    )

                if (!isNull(name)) this.theme.active = this.theme.registry.get(name)
            ...
            resolve: (scopes = [], name, def) => {
                let props = isNull(this.theme.active)
                    ? {}
                    : merge(
                          {},
                          ...this.theme.active.settings
                              .filter(({ scope }) => {
                                  return (
                                      isNil(scope) ||
                                      scopes.some(
                                          other => scope === other || other.startsWith(`${scope}.`)
                                      )
                                  )
                              })
                              .map(({ settings }) => {
                ...
                if (isUndefined(name)) return props

                if (!isUndefined(props[name])) return props[name]
        ...
        if (true) {
            ...
            if (tokens.length === 0) {
                ...
                let bottomRowSpaceCount =
                    row < this.rawLines.length - 2
                        ? this.rawLines[row + 1].match(/^ */)[0].length
                        : 0
