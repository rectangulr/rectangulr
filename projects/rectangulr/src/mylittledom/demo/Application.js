...
export class Application extends React.PureComponent {
    ...
    componentDidMount() {
        ...
        let moduleName = getPreset
            .keys()
            .find(moduleName => getPresetName(moduleName) === presetName)
    ...
    setRunningCode(code) {
        ...
        try {
            ...
            let transpiled = `(function (env) { with(env) {${
                transform(code, {
            ...
            if (this.screen) this.screen.releaseScreen()

            this.screen = window.screen = screen
            this.screen.attachScreen({
                stdin: this.refs.terminal.stdin,
                stdout: this.refs.terminal.stdout,
            })
                        ...
                        <select
                            ...
                            value={
                                getPreset
                                    .keys()
                                    .find(
                                        moduleName => getPreset(moduleName) === this.state.code
                                    ) || `custom`
                            }
                        >
                            {getPreset.keys().map(moduleName => (
                                <option key={moduleName} value={moduleName}>
                                    {getPresetName(moduleName)}
                                </option>
                            ))}
                            <option
                                disabled={true}
