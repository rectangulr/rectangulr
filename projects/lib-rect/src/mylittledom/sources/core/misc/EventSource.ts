...
export class EventSource {
  ...
  static setup(instance, extra?) {
    let eventSource = (instance[EventSource.symbol] = new this(
      instance,
      extra
    ));
  ...
  getParentEventSource() {
    ...
    return parent && parent[EventSource.symbol]
      ? parent[EventSource.symbol]
      : null;
  }
  ...
  hasDeclaredEvent(name) {
    if (!isString(name))
      throw new Error(
        `Failed to execute 'hasDeclaredEvent': Parameter 1 is not of type 'string'.`
      );
  ...
  declareEvent(name) {
    if (!isString(name))
      throw new Error(
        `Failed to execute 'declareEvent': Parameter 1 is not of type 'string'.`
      );

    if (this.listeners.has(name))
      throw new Error(
        `Failed to execute 'declareEvent': '${name}' already exists.`
      );
  ...
  addEventListener(name, callback, { capture = false, once = false } = {}) {
    if (!isString(name))
      throw new Error(
        `Failed to execute 'addEventListener': Parameter 1 is not of type 'string'.`
      );
    ...
    if (!this.listeners.has(name))
      throw new Error(
        `Failed to execute 'addEventListener': '${name}' is not a valid event name.`
      );
  ...
  dispatchEvent(event, { parentSource = this.getParentEventSource() } = {}) {
    ...
    for (let t = 0, T = eventSources.length; t < T; ++t) {
      ...
      for (let [callback, { once }] of [
        ...listeners.capture,
        ...allListeners.capture,
      ]) {
    ...
    for (
      let t = 0, T = Math.max(0, event.bubbles ? eventSources.length : 1);
      t < T;
      ++t
    ) {
      if (event.propagationStopped) break;
