...
export class Node {
    ...
    get firstChild() {
        if (isEmpty(this.childNodes)) return null
    ...
    appendTo(node) {
        ...
        if (wouldContainItself(this, node))
            throw new Error(
                `Failed to execute 'appendTo': The new child element contains the parent.`
            )
    ...
    appendChild(node) {
        ...
        if (wouldContainItself(node, this))
            throw new Error(
                `Failed to execute 'appendChild': The new child element contains the parent.`
            )
