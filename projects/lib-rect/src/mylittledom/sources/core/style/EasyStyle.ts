...
export function EasyStyle(ruleset: Ruleset, selector = [], base = Object.create(null)) {
    ...
    return new Proxy(base, {
        ...
        get(target, key: string, receiver) {
            if (has(base, key)) return base[key]
        ...
        set(target, key: string, value, receiver) {
            ...
            else if (!isUndefined(value)) assign(new Map([[key, parsePropertyValue(key, value)]]))
        ...
        deleteProperty(target, key) {
            if (!has(styleProperties, key))
                throw new Error(
                    `Failed to delete a style property: '${String(
                        key
                    )}' is not a valid style property name.`
                )
