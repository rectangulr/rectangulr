...
export function repeat(n, parsers) {
    return rawValue => {
        ...
        if (value.some(sub => isUndefined(sub))) return undefined
...
export function number(rawValue) {
    if (!isNumber(rawValue) && !isString(rawValue)) return undefined
...
length.rel = function (rawValue) {
    ...
    if (!isString(rawValue) || !rawValue.endsWith(`%`)) return undefined
...
export function character(rawValue) {
    if (!isString(rawValue) || rawValue.length !== 1) return undefined
