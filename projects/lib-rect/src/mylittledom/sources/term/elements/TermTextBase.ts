...
export class TermTextBase extends TermElement {
    styleManager: any
    textBufferCallback: any
    disposeTextBufferCallback: any
    textLayout: any
    textBuffer: any
    yogaNode: any
    contentWorldRect: any
    scrollRect: any
    caretIndex: any
    activeElement: this
    caret: Point
    ...
    constructor({
        ...
        this.setPropertyTrigger(`textBuffer`, textBuffer, {
            trigger: value => {
                ...
                if (value) {
                    this.disposeTextBufferCallback = value.onDidChange(
                        ({ oldRange, oldText, newText }) => {
                            ...
                            let oldEnd = this.textLayout.getPositionForCharacterIndex(
                                offset + oldText.length
                            )
                            // let patch = this.textLayout.update(offset, oldText.length, newText.length);
                            TL.applyPatch(this.textLayout, this.textLayout.setSource(newText), [''])
                            let newEnd = this.textLayout.getPositionForCharacterIndex(
                            ...
                            if (oldText.length !== newText.length) {
                                this.caretIndex =
                                    this.rootNode.activeElement === this
                                        ? this.textBuffer.getMaxCharacterIndex()
                                        : 0
                                this.caret = new Point(
                                    this.textLayout.getPositionForCharacterIndex(this.caretIndex)
                                )
        ...
        this.setPropertyTrigger(`textLayout`, textLayout, {
            trigger: value => {
                if (value) {
                    ...
                    TL.applyPatch(this.textLayout, this.textLayout.setSource(''), [''])

                    value.setSoftWrap(this.style.$.whiteSpace.doesWrap)

                    value.setDemoteNewlines(this.style.$.whiteSpace.doesDemoteNewlines)

                    value.setCollapseWhitespaces(this.style.$.whiteSpace.doesCollapse)

                    value.setPreserveLeadingSpaces(!this.style.$.whiteSpace.doesCollapse)

                    value.setPreserveTrailingSpaces(!this.style.$.whiteSpace.doesCollapse)

                    value.setJustifyText(this.style.$.textAlign.isJustified)

                    value.setAllowWordBreaks(this.style.$.overflowWrap.doesBreakWords)
                }
        ...
        this.addShortcutListener(
            ...
            e => {
                ...
                e.setDefault(() => {
                    ...
                    this.caret.x = this.caretMaxColumn
                    this.caret = new Point(
                        this.textLayout.getPositionAbove(this.caret, this.elementRect.height)
                    )
                    this.caretIndex = this.textLayout.getCharacterIndexForPosition(this.caret)
        ...
        this.addShortcutListener(
            ...
            e => {
                ...
                e.setDefault(() => {
                    ...
                    this.caret.x = this.caretMaxColumn
                    this.caret = new Point(
                        this.textLayout.getPositionBelow(this.caret, this.elementRect.height)
                    )
                    this.caretIndex = this.textLayout.getCharacterIndexForPosition(this.caret)
        ...
        this.addShortcutListener(
            `enter`,
            e => {
                if (this.enterIsNewline && !this.caret) return

                if (this.enterIsNewline && this.readOnly) return
                ...
                e.setDefault(() => {
                    if (this.enterIsNewline) {
                        ...
                        this.textBuffer.insert(
                            this.textBuffer.positionForCharacterIndex(this.caretIndex),
                            `\n`
                        )
    ...
    renderContent(x, y, l) {
        if (this.textLayout.rows <= y) return this.renderBackground(l)
