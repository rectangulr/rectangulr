...
export class StyleManager {
  ...
  element: TermElement
  states: Set<string>
  nativeRulesets: Set<Ruleset>
  userRulesets: Set<Ruleset>
  ...
  getStyle() {
    ...
    return EasyStyle(localRuleset, [], {
      $: EasyComputedStyle(this.computed),
      ...
      when(selector) {
        return EasyStyle(localRuleset, parseSelector(selector) as any, {
          assign(propertyValues) {
            Object.assign(this, propertyValues)
          },
        })
  ...
  setStateStatus(state: string, status) {
    if (status) {
  ...
  setRulesets(rulesets: Set<Ruleset>, target = StyleManager.RULESET_USER) {
  ...
  addRuleset(ruleset, target = StyleManager.RULESET_USER) {
    ...
    switch (target) {
      case StyleManager.RULESET_NATIVE:
        {
          if (this.nativeRulesets.has(ruleset)) return

          if (this.userRulesets.has(ruleset))
            throw new Error(
              `Failed to execute 'addRuleset': This ruleset already has been registered as a user ruleset.`
            )
      ...
      case StyleManager.RULESET_USER:
        {
          if (this.userRulesets.has(ruleset)) return

          if (this.nativeRulesets.has(ruleset))
            throw new Error(
              `Failed to execute 'addRuleset': This ruleset already has been registered as a native ruleset.`
            )
  ...
  @autobind handleRulesetChange(e) {
    for (let state of e.states) if (!this.states.has(state)) return
    ...
    for (let rulesets of this.stylePasses) {
      ...
      for (let ruleset of rulesets) {
        ruleLoop: for (let { states, propertyValues } of ruleset.rules) {
          if (!propertyValues.has(propertyName)) continue ruleLoop // it doesn't have the property we're computing

          if (states.size > this.states.size) continue ruleLoop // it cannot match anyway

          let ruleSpecificity = getSpecificity(states)

          if (ruleSpecificity < specificity) continue ruleLoop // it has a lower specificity than ours

          for (let state of states) if (!this.states.has(state)) continue ruleLoop
  ...
  refresh(propertyNames, { inheritedOnly = false } = {}) {
    ...
    for (let propertyName of propertyNames) {
      if (inheritedOnly && !this.inherited.has(propertyName)) continue
      ...
      if (!_.isEqual(serializePropertyValue(newValue), serializePropertyValue(oldValue))) {
        dirtyPropertyNames.add(propertyName)
