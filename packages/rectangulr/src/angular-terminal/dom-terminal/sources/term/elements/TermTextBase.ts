...
export class TermTextBase extends Element {
  styleManager: any
  ...
  constructor({
        ...
        if (value) {
          this.disposeTextBufferCallback = value.onDidChange(({ oldRange, oldText, newText }) => {
            ...
            if (oldText.length !== newText.length) {
              this.caretIndex =
                this.rootNode.activeElement === this ? this.textBuffer.getMaxCharacterIndex() : 0
    ...
    this.addEventListener(`layout`, () => {
      if (this.style.$.display.serialize() == 'flex') {
        this.textLayout.setLayoutConfig({ maxWidth: this.contentRect.width })
      }
    })
  ...
  getPreferredSize(maxWidth) {
    this.textLayout.setLayoutConfig({ maxWidth: maxWidth })
