...
const Point_1 = require("../misc/Point");
const Rect_1 = require("../misc/Rect");
const globalRuleset_1 = require("../style/globalRuleset");
const StyleManager_1 = require("../style/StyleManager");
const runPropertyTriggers_1 = require("../style/tools/runPropertyTriggers");
...
function mergeNewStyles(node) {
    for (const [style, value] of Object.entries(this.newStyles)) {
        runPropertyTriggers_1.runPropertyTriggers(style, this, value, this.style[style] ?? null);
    }
...
class Element extends Node_1.Node {
    constructor({ classList = [], style = {}, caret = null, decored = true } = {}) {
        super();
        this.flags = flags_1.flags.ELEMENT_HAS_DIRTY_NODE_LIST | flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT;
        this.dirtyRects = [];
        this.nodeList = [];
        this.focusList = [];
        this.renderList = [];
        ...
        this.scrollRect = new Rect_1.Rect();
        /** Position & size of the element inside the viewport */
        this.elementWorldRect = new Rect_1.Rect();
        /** Position & size of the element content inside the viewport */
        this.contentWorldRect = new Rect_1.Rect();
        ...
        EventSource_1.EventSource.setup(this, { getParentInstance: () => this.parentNode });
        this.yogaNode = Yoga.Node.createWithConfig(yogaConfig);
        this.yogaNode.setMeasureFunc(getPreferredSize.bind(null, this));
        this.styleManager = new StyleManager_1.StyleManager(this);
        ...
        this.setPropertyTrigger('caret', caret, {
            validate: value => value === null || value instanceof Point_1.Point,
            trigger: value => {
        ...
        this.styleManager.addRuleset(globalRuleset_1.globalRuleset, StyleManager_1.StyleManager.RULESET_NATIVE);
        this.styleManager.setRulesets(new Set(), StyleManager_1.StyleManager.RULESET_USER);
    ...
    removeChild(node) {
        ...
        if (this.rootNode.activeElement &&
            (this.rootNode.activeElement === node || traverse_1.isChildOf(this.rootNode.activeElement, node))) {
    ...
    set scrollLeft(scrollLeft) {
        ...
        if (previousScrollLeft !== newScrollLeft) {
            ...
            this.setDirtyClippingFlag();
            this.queueDirtyRect();
            this.dispatchEvent(new Event_1.Event('scroll'));
        }
    }
    get scrollTop() {
        this.triggerUpdates();
    ...
    getCaretCoordinates() {
        if (this.rootNode !== this) {
            ...
            if (x < this.activeElement.contentClipRect.x ||
                ...
                return null;
            if (y < this.activeElement.contentClipRect.y ||
    ...
    focus() {
        ...
        this.scrollIntoView();
        this.dispatchEvent(new Event_1.Event('focus', {}, { was: previousActiveNode }));
        this.rootNode.requestUpdates();
    }
    blur({ parentNode = this.parentNode } = {}) {
        ...
        this.dispatchEvent(new Event_1.Event('blur'), { parentSource: parentNode.getEventSource() });
    ...
    scrollIntoView({ align = 'auto', alignX = align, alignY = align, force = false, forceX = force, forceY = force, } = {}) {
        ...
        if (this.caret) {
            ...
            this.parentNode.scrollCellIntoView(new Point_1.Point({ x, y }), {
                alignX,
                alignY,
                forceX,
                forceY,
    ...
    scrollCellIntoView(position, { align = 'auto', alignX = align, alignY = align, force = false, forceX = force, forceY = force, } = {}) {
        ...
        if (this.parentNode) {
            ...
            this.parentNode.scrollCellIntoView(new Point_1.Point({ x, y }), { alignX, alignY });
        }
    }
    scrollColumnIntoView(column, { align = 'auto', force = false } = {}) {
        this.scrollCellIntoView(new Point_1.Point({ x: column, y: this.scrollTop }), {
            alignX: align,
            forceX: force,
        });
    }
    scrollRowIntoView(row, { align = 'auto', force = false } = {}) {
        this.scrollCellIntoView(new Point_1.Point({ x: this.scrollLeft, y: row }), {
            alignY: align,
            forceY: force,
        });
    }
    setDirtyNodeListFlag() {
        this.setDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_NODE_LIST);
    }
    clearDirtyNodeListFlag() {
        this.clearDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_NODE_LIST);
    }
    setDirtyFocusListFlag() {
        this.setDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_FOCUS_LIST);
    }
    clearDirtyFocusListFlag() {
        this.clearDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_FOCUS_LIST);
    }
    setDirtyRenderListFlag() {
        this.setDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_RENDER_LIST);
    }
    clearDirtyRenderListFlag() {
        this.clearDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_RENDER_LIST);
    }
    setDirtyLayoutFlag() {
        this.setDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT, flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT_CHILDREN);
    }
    clearDirtyLayoutFlag() {
        this.clearDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT);
    }
    clearDirtyLayoutChildrenFlag() {
        this.clearDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT_CHILDREN);
    }
    setDirtyClippingFlag() {
        this.setDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING, flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING_CHILDREN);
    }
    clearDirtyClippingFlag() {
        this.clearDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING);
    }
    clearDirtyClippingChildrenFlag() {
        this.clearDirtyFlag(flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING_CHILDREN);
    }
    ...
    queueDirtyRect(dirtyRect = this.elementClipRect, checkIntersectionFrom = 0) {
        if (Rect_1.Rect.isEmpty(dirtyRect))
            return;
        if (this.rootNode !== this)
            return this.rootNode.queueDirtyRect(Rect_1.Rect.getIntersectingRect(dirtyRect, this.elementClipRect), checkIntersectionFrom);
    ...
    triggerUpdates({ maxDepth = 5 } = {}) {
        ...
        let needsFullRerender = this.flags & (flags_1.flags.ELEMENT_HAS_DIRTY_NODE_LIST | flags_1.flags.ELEMENT_HAS_DIRTY_RENDER_LIST);
        if (this.flags & flags_1.flags.ELEMENT_HAS_DIRTY_NODE_LIST) {
            this.nodeList = this.generateNodeList();
            this.clearDirtyNodeListFlag();
        }
        if (this.flags & flags_1.flags.ELEMENT_HAS_DIRTY_FOCUS_LIST) {
            this.focusList = this.generateFocusList();
            this.clearDirtyFocusListFlag();
        }
        if (this.flags & flags_1.flags.ELEMENT_HAS_DIRTY_RENDER_LIST) {
            this.renderList = this.generateRenderList();
            this.clearDirtyRenderListFlag();
        }
        let dirtyLayoutNodes = [];
        let dirtyScrollNodes = [];
        if (this.flags & (flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT | flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT_CHILDREN)) {
            this.yogaNode.calculateLayout();
            this.cascadeLayout({ dirtyLayoutNodes });
        }
        if (this.flags & (flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING | flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING_CHILDREN))
            this.cascadeClipping({ dirtyScrollNodes });
        if (this.flags & flags_1.flags.ELEMENT_IS_DIRTY)
            throw new Error(`Aborted 'triggerUpdates' execution: Flags have not been correctly reset at some point (0b${(this.flags & flags_1.flags.ELEMENT_IS_DIRTY)
                .toString(2)
                .padStart(16, '0')}).`);
        for (let dirtyLayoutNode of dirtyLayoutNodes)
            dirtyLayoutNode.dispatchEvent(new Event_1.Event('layout'));
        for (let dirtyScrollNode of dirtyScrollNodes)
            dirtyScrollNode.dispatchEvent(new Event_1.Event('scroll'));
    ...
    generateNodeList() {
        ...
        while (!lodash_1.isEmpty(traverseList)) {
    ...
    generateRenderList() {
        ...
        while (stackingContexts.length > 0) {
            ...
            while (childNodes.length > 0) {
                ...
                if (!lodash_1.isNull(child.style.$.zIndex)) {
    ...
    cascadeLayout({ dirtyLayoutNodes = [], force = false } = {}) {
        if (force ||
            this.flags & (flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT | flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT_CHILDREN)) {
            let doesLayoutChange = false;
            let doesScrollChange = false;
            if (force || this.flags & flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT) {
                ...
                doesLayoutChange = !Rect_1.Rect.areEqual(this.elementRect, prevElementRect);
            }
            if (this.flags & (flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT | flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT_CHILDREN) ||
                doesLayoutChange) {
                for (let child of this.childNodes)
            ...
            if (this.flags & flags_1.flags.ELEMENT_HAS_DIRTY_LAYOUT || doesLayoutChange || doesScrollChange) {
    ...
    cascadeClipping({ dirtyScrollNodes = [], relativeClipRect = null, force = false } = {}) {
        if (force ||
            this.flags & (flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING | flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING_CHILDREN)) {
            if (force || this.flags & flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING) {
                ...
                this.elementClipRect = relativeClipRect
                    ? Rect_1.Rect.getIntersectingRect(this.elementWorldRect, relativeClipRect)
                    : this.elementWorldRect;
                this.contentClipRect = relativeClipRect
                    ? Rect_1.Rect.getIntersectingRect(this.contentWorldRect, relativeClipRect)
                    : this.contentWorldRect;
                doesClippingChange =
                    !Rect_1.Rect.areEqual(prevElementWorldRect, this.elementWorldRect) ||
                        !Rect_1.Rect.areEqual(prevContentWorldRect, this.contentWorldRect) ||
                        !Rect_1.Rect.areEqual(prevElementClipRect, this.elementClipRect);
            ...
            for (let child of this.childNodes)
                child.cascadeClipping({
                    dirtyScrollNodes,
                    relativeClipRect,
                    force: force || this.flags & flags_1.flags.ELEMENT_HAS_DIRTY_CLIPPING,
                });
            this.elementBoundingRect = Rect_1.Rect.getBoundingRect(this.elementClipRect, ...this.childNodes.map(child => child.elementBoundingRect));
            this.clearDirtyClippingFlag();
            this.clearDirtyClippingChildrenFlag();
        }
    }
    getElementRects() {
        return lodash_1.pick(this, [
            'elementRect',
            'contentRect',
            'elementWorldRect',
            'contentWorldRect',
...
__decorate([
    core_decorators_1.override
], Element.prototype, "linkBefore", null);
__decorate([
    core_decorators_1.override
