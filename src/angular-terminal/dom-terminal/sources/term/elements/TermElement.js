...
exports.TermElement = void 0;
const term_strings_1 = require("@manaflair/term-strings");
const core_1 = require("../../core");
const KeySequence_1 = require("../misc/KeySequence");
class TermElement extends core_1.Element {
    constructor(props) {
        super(props);
        // // Init style
        ...
        this.addEventListener(`mouseup`, e => {
            ...
            e.setDefault(() => {
                let event = new core_1.Event(`click`, { bubbles: true });
                event.mouse = e.mouse;
    ...
    addShortcutListener(descriptors, callback, { capture = false } = {}) {
        ...
        for (let descriptor of descriptors.split(/,/g)) {
            let sequence = new KeySequence_1.KeySequence(descriptor);
    ...
    click(mouse) {
        this.dispatchEvent(Object.assign(new core_1.Event(`click`), { mouse }));
    }
    renderElement(x, y, l) {
        let processBorders = (x, y, l) => {
            ...
            if (y === 0 && this.style.$.borderTopCharacter) {
                ...
                if (!this.rootNode.debugPaintRects &&
                    ...
                    data += term_strings_1.style.clear;
                return data;
            }
                ...
                if ((this.style.$.backgroundColor && this.style.$.backgroundClip.doesIncludeBorders) ||
                    this.style.$.borderColor) {
                    if (prepend)
                        prepend += term_strings_1.style.clear;
                    if (append) {
                        append += term_strings_1.style.clear;
                    }
                }
                return (prepend +
    ...
    renderBackground(l) {
        ...
        if (this.style.$.backgroundColor || this.style.$.color)
            background += term_strings_1.style.clear;
        return background;
    }
    renderText(text) {
        if (this.rootNode.debugPaintRects)
        ...
        if (this.style.$.fontWeight < 400)
            prefix += term_strings_1.style.fainted.in;
        else if (this.style.$.fontWeight > 400)
            prefix += term_strings_1.style.emboldened.in;
        if (this.style.$.textDecoration && this.style.$.textDecoration.isUnderlined)
            prefix += term_strings_1.style.underlined.in;
        if (this.style.$.backgroundColor)
            prefix += this.style.$.backgroundColor.back;
        if (this.style.$.color)
            prefix += this.style.$.color.front;
        if (prefix.length !== 0)
            suffix += term_strings_1.style.clear;
        return prefix + text + suffix;
    }
}
exports.TermElement = TermElement;
